function afficherAliments(idPlat) {
  $(document).ready(function () {
    var tabAliment = $("#alimentTable").DataTable({
      ajax: {
        url: apiPath + "backend/aliments.php",
        dataSrc: ''
      }, columnDefs: [
        {
          orderable: false,
  ...
  if (requestVerifType3.readyState == 4) {
    if (requestVerifType3.status != 200) {
      ...
      if (JSON.parse(requestVerifType3.response).length == 0) {
        ...
        const idPlat = JSON.parse(requestVerifType3.response)[0].ID_ALIMENT
        afficherAliments(idPlat);
        addToSup(idPlat);
      }
    }
  }
...
function ajoutAliment(idAli, idPlat) {
  ...
  fetch(apiPath + 'backend/contenir.php', {
    method: 'POST',
    headers: {
...
function supprimerAliment(idAli, idPlat) {
  but = document.getElementById("button-" + idAli);
  but.disabled = true;
  fetch(apiPath + `backend/contenir.php?id_aliment=${idPlat}&ali_id_aliment=${idAli}`, {
    method: 'DELETE',
    headers: {
      'Content-Type': 'application/json'
    }
